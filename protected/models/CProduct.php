...
class CProduct extends CActiveRecord
{
    ...
    public static function getPartnerProductsForUser($search = '', $partner_id = 0, $page = 1, $count = 10)
    {
        ...
        $cmd = Yii::app()->db->createCommand()
            ->select('p.id, p.title AS title,pv.title as pvtitle, prt.id AS prtid, prt.title AS prttitle, pv.id AS variant_id, COALESCE(ppv.value,"http://safelib.com/images/films/noposter.jpg") as image, COALESCE(ppvT.value,"-")  as original_title,COALESCE(ppvY.value,"-")  as year,COALESCE(ppvG.value,"-")  as genre,COALESCE(ppvC.value,"-")  as country')
            ->from('{{products}} p')
    ...
    public static function getTopProducts($search = '', $partner_id = 0, $page = 1, $count = 10)
    {
        $offset = ($page - 1) * $count;
        $searchCondition = '';
        if (!($search == '')) {
    ...
    public static function countTopProducts($search = '', $partner_id = 0)
    {
        $searchCondition = '';
        if (!($search == '')) {
            $searchCondition = ' AND (p.title LIKE "%' . $search . '%" OR ppvT.value LIKE "%' . $search . '%")';
            ...
            if ($product){
                ...
                foreach ($product['variants'] as &$variant){
                    $variant['items'] = Yii::app()->db->createCommand()
                        ...
                        ->leftjoin('{{typedfiles}} tf', 'tf.variant_id =vq.variant_id and tf.variant_quality_id = COALESCE(vq.preset_id,0) AND tf.user_id = ' . Yii::app()->user->id) // TO DO: WHY tf.variant_quality_id not same as pf.variant_quality_id??

...
class AppController extends ControllerApp
{
        ...
        if (isset($_REQUEST['email']) && isset($_REQUEST['password'])) {
            ...
            if ($email) {
                ...
                if (!$user) {
                    ...
                    $user->group_id = self::USERGROUP_USER;
    ...
    {
        if (isset($_REQUEST['variant_id']) && isset($_REQUEST['quality_id'])) {
            $variant_id = (int)$_REQUEST['variant_id'];
            $quality_id = (int)$_REQUEST['quality_id'];
            if ($res = CProduct::addProductToUser($variant_id,$quality_id)) {
                echo json_encode(array('cmd' => "AddItemFromPartner", 'error' => self::ERROR_NONE, 'cloud_id' => $res));
            } else
                echo json_encode(array('cmd' => "AddItemFromPartner", 'error' => self::ERROR_ADD_FAILED));
    } else
    ...
    */
    ...
    public
    function actionUserProductList($content_type = self::CONTENT_TYPE_VIDEO)
    {
    ...
    public
    function actionUserProductData($item_id = 0)
    {
    ...
    public
    function actionPartnerProducts($partner_id = 0)
    {
        $list = CProduct::getPartnerProductsForUser($this->search, $partner_id, $this->page, $this->per_page);
        $count = count($list);
        $total_count = CProduct::CountPartnerProductsForUser($this->search, $partner_id);
    ...
    public
    function actionPartnerProductData($variant_id = 0)
    {
    ...
    public
    function actionUserLibraryList($content_type = self::CONTENT_TYPE_VIDEO)
    {
    ...
    public
    function actionUserLibraryItemData($item_id = 0)
    {
        $data = CUserObjects::getUserObject($item_id, Yii::app()->user->id);
        if (!empty($data)) {
            $result = array('cmd' => "ItemData", 'error' => self::ERROR_NONE, 'Data' => $data[0]);
    ...
    public
    function actionUserQueueData($item_id = 0)
    {
        $data = CConvertQueue::getUserObject($item_id, Yii::app()->user->id);
        if (!empty($data)) {
            $result = array('cmd' => "UserQueueData", 'error' => self::ERROR_NONE, 'Data' => $data[0]);
    ...
    public
    function actionUserFileData($item_id = 0)
    {
        $data = CConvertQueue::getUserObject($item_id, Yii::app()->user->id);
        if (!empty($data)) {
            $result = array('cmd' => "UserQueueData", 'error' => self::ERROR_NONE, 'Data' => $data[0]);

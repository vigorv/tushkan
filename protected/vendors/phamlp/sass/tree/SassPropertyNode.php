...
class SassPropertyNode extends SassNode {
	...
	public static function isa($token, $syntax) {
		...
		if (!empty($matches)) {	
	  	...
	  	if ($token->level === 0) {//nlac: this drops error for such selector:   :focus{...}
	  		throw new SassPropertyNodeException('Properties can not be assigned at root level', array(), $token);//nlac: $this -> $token
	  	}
	  	else {
				return true;
	  	}

<?php
Yii::import('ext.classes.EBBCode');
...
class BBCodeTest extends CTestCase
{
	...
	function testSingleTags()
	{
		...
		$this->assertEquals('<a href="http://yiiframework.com/">http://yiiframework.com/</a>',
			$this->process('[url]http://yiiframework.com/[/url]')
		);
		$this->assertEquals('<a href="http://yiiframework.com/">yiiframework.com</a>',
	...
	{
		$this->assertEquals('<strong>test1</strong> <strong>test2</strong>',
			$this->process('[b]test1[/b] [b]test2[/b]')
		);

		$this->assertEquals('<strong><em>test</em></strong>',
